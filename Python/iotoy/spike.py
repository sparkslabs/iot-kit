...
def http_debug(request):
    ...
    todump = [("Server", "%(SERVER_NAME)s:%(SERVER_PORT)s%(PATH_INFO)s" % request.environ),
...
class WebInterface(threading.Thread):
    ...
    def __init__(self, thing):
        ...
        self.thing = thing  # Provided as a utility for things that don't want/need to provide a custom __init__ function
        self.ready = False
        self.app = None

    def run(self):
        self.app = app = Flask(__name__)
        baremethods = [x for x in self.__class__.__dict__.keys() if x[0] != "_" and x not in ("daemon","run")]
...
def mdns_lookup(requestedname = "oven", regtype  = "_iotoy._tcp", suffix=".local."):
    ...
    def LOOKUP__query_record_callback(sdRef, flags, interfaceIndex, errorCode, fullname, rrtype, rrclass, rdata, ttl):
        if errorCode == pybonjour.kDNSServiceErr_NoError:
            print '  IP         =', socket.inet_ntoa(rdata)
            queried.append(socket.inet_ntoa(rdata))


    def LOOKUP__resolve_callback(sdRef, flags, interfaceIndex, errorCode, fullname, hosttarget, port, txtRecord):
        if errorCode != pybonjour.kDNSServiceErr_NoError:
            return
        ...
        try:
            while not queried:
                ...
                IP = queried.pop()
                if fullname == requestedfullname: # This acts as a filter.If we didn't do this we'd have to do a time based thing instead, which may be more appropriate
                    print fullname
    ...
    def LOOKUP__browse_callback(sdRef, flags, interfaceIndex, errorCode, serviceName, regtype, replyDomain):
        if errorCode != pybonjour.kDNSServiceErr_NoError:
            return
    ...
    browse_sdRef = pybonjour.DNSServiceBrowse(regtype = regtype, callBack = LOOKUP__browse_callback)
        ...
        try:
            while True:
                ...
                if browse_sdRef in ready[0]:
                    ...
                    raise Exception("Main Fail - means not found, let alone resolvable or queryable")
                if len(results)>0:
                    print results
                    break
...
class ActualOven(threading.Thread): # Well, obviously not a "real" oven, simulation of an oven
    """This simulates an oven that we're communicating with over a serial connection. In particular, it simulates what we're communicating to."""
    """If there was an arduino on the system, this would be the arduino part"""
    daemon = True
            ...
            if raw_command:
                ...
                if command[0] == "SET":
                    if command[1] == "target":
                        ...
                        self.fake_serial.write("200 target "+ str(self._target)+"\n")
                    else:
...
#############################################################################################
#
# SYSTEM PROXY - FOR INTERFACING WITH THE REAL OBJECT - EITHER DIRECTLY OR VIA SERIAL COMMS
#
#############################################################################################

class Oven(object):
    "This is the local proxy for the physical device, that the network proxy uses to control the device"
    daemon = True
    def __init__(self, ser): # Ser is the serial port we're using to communiate with the IOT-able object
        self.ser = ser


    def get_targettemperature(self):
        """This is the temperature that the oven is currently set to. This is always a target,
           not reality This could be a read-write property really"""
    ...
    def temperature(self):
        "This is the temperature that the oven currently is. You can't change this, just the target temp."
...
class OvenInterface(WebInterface):
    ...
    def PUT__target_temperature(self):
        ...
        if set_to == data["value"]:
            return jsonify({"_meta" : IOTOY_INT, "value":self.thing.get_targettemperature()})
        else:
            # Gateway error of somekind, setting failed
            return jsonify({"_meta" : IOTOY_INT, "value":set_to})
    ...
    def GET__temperature(self):
        return jsonify({"_meta" : IOTOY_INT, "value":self.thing.temperature()})

    def GET__(self):
        http_debug(request)
        #
        # FIXME: Response should be:
        # { "_meta" : IOTOY_DIR, "value" : ["temperature", "target_temperature"] }
        #
#        response = make_response(json.dumps(["temperature", "target_temperature"]))
        response = make_response(json.dumps({ "_meta" : IOTOY_DIR, "value" : ["temperature", "target_temperature"] }))
...
#############################################################################################
#
# Client side code proxy for the mDNS advertised Web API. Provides a programmer friendly
# API which maps Python object access attributes to appropriate RESTful calls
#
#############################################################################################

class oven_proxy(object):
    "This is a proxy object for the real IOT device, wrapping up how the user communicates with it over HTTP"
    def __init__(self, oven_name):

        services = mdns_lookup(requestedname = oven_name, regtype  = "_iotoy._tcp", suffix=".local.")
    ...
    def _set_target(self, value):
        result = requests.put(self.service + "/target_temperature", data=json.dumps({ "_meta" : IOTOY_INT, "value": value }),
    ...
    target_temperature = property(_get_target, _set_target) # This is a property here, but should really be implemented using getattr things in some shape or form.
...
def props(thing):
    "This function allows us to inspect IOT proxy objects to find out what they can do..."
    thing.__dict__.keys()
    thing.__class__.__dict__.keys()
    return [ x for x in list(set(thing.__dict__.keys() + thing.__class__.__dict__.keys())) if x[0] != "_"]
...
#    * Client side code that monitors the oven and makes it change temperatures at
#      given times.
#
# Currently Missing:
#    * __call__ - mapping to post
#    * Object data model - based on jsonpickle & .well-known / meta ideas & JSON HOME proposed standard
#    * Non-local external proxy via httptunnel or similar. (easy to bolt on, but needs integrating)
