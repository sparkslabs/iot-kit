...
class SimpleDFULoader(QtGui.QWidget):

    def __init__(self):
        super(SimpleDFULoader, self).__init__()
        self.initUI()

    def initUI(self):
        self.btn = QtGui.QPushButton('PROGRAM DEVICE!', self)
        self.btn.move(20, 20)
        self.btn.clicked.connect(self.showDialog)
        self.btn.setGeometry(416, 295, 180, 30)

        # pixmap = QtGui.QPixmap("Instructions.png")

        # palette = QPalette()
        # palette.setBrush(QPalette.Background,QBrush(pixmap))
        # self.setPalette(palette)

        self.setWindowTitle('Simple DFU Loader')
    ...
    def checkHexfileIsDFUFile(self): # This could be checked by looking for a magic string sequence
        return True
    ...
    def showDialog(self):
        ...
        if fname:
            #self.waitDeviceReady() # Only works on Linux
            self.eraseDevice()
